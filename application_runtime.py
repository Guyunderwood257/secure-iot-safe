...
MASTER_PIN_HASH = ubinascii.unhexlify(
    "d35258b5938aa832236fea9dcae1ddd0f6094afac9dd5aef037f4ae4ed006c31"
)
...
from mykeys_config import (
    VERIFICATION_CODE_SECRET,
    TELEMETRY_CIPHER_KEY,
    LOCAL_AES_KEY
)

# Decoy PIN used for deception-based trap behaviour
HONEYPOT_PIN_HASH = ubinascii.unhexlify(
    "26f3cc2b30060fa74871aad1ae08332cf14383e2674352ae2f2814d603a3a860"
)
...
def base32_decode(text):
    # Decode the Base32 secret used for verification-code generation
...
def hmac_sha1(key, msg):
    # Build an HMAC-SHA1 digest for telemetry and TOTP use
    block_size = 64
...
def constant_time_compare(a, b):
    # Compare values in constant time to reduce timing-attack leakage
    if len(a) != len(b):
        return False
...
def unix_time_now():
    # Convert the MicroPython epoch to Unix time for TOTP compatibility
    now = int(time.time())
...
class RiskEngine:
    # Lightweight on-device logistic-regression risk model
    ...
    def __init__(self, learning_rate=0.12):
        ...
        self.weights = {
            "wrong_pin_count":            0.0,
            "fingerprint_fail_count":     0.0,
            "lockout_count":              0.0,
            "rapid_retry_flag":           0.0,
            "repeated_session_fail_flag": 0.0
        }

    def _sigmoid(self, z):
        # Sigmoid activation with simple clamping at extreme values
        if z < -20:
            return 0.0
        if z > 20:
            return 1.0
    ...
    def _features(self, wrong_pin, fp_fail, lockout, rapid, repeated):
        # Scale behavioural features into a 0-1 range
        return {
            "wrong_pin_count":            min(wrong_pin / 5.0, 1.0),
            "fingerprint_fail_count":     min(fp_fail / 5.0, 1.0),
            "lockout_count":              min(lockout / 3.0, 1.0),
            "rapid_retry_flag":           float(rapid),
            "repeated_session_fail_flag": float(repeated)
        }
    ...
    def evaluate(self, wrong_pin, fp_fail, lockout, rapid, repeated):
        # Return the current risk score and corresponding risk level
    ...
    def train(self, wrong_pin, fp_fail, lockout, rapid, repeated, label):
        # Update the model after one labelled authentication outcome
        features = self._features(wrong_pin, fp_fail, lockout, rapid, repeated)
...
class SecurityControlSystem:
    ...
    def print_attempt_log(self):
        ...
        print("{:<{}} {}".format(
            "CHAIN",
            LOG_LABEL_WIDTH,
            ubinascii.hexlify(self.log_chain_hash).decode()
        ))
        print("===========================================")
        print("")
    ...
    def _log_event(self, event_code, event_text, reason=REASON_NONE,
                   extra="", fp=0, lock=1, access_result=0, fa=None):
        # Keep cloud telemetry and local logging aligned for the same event
        base = "{};reason={};risk={};score={};total_failed={}".format(
            event_text,
            reason,
        ...
        if telemetry is not None:
            telemetry.queue_event(
                event_code,
                self.failed_attempts if fa is None else fa,
                fp,
                lock,
                access_result,
                encrypt_status(status),
                risk_score=self.current_risk_score,
                lockout_count=self.total_lockout_count,
                fp_failures=self.fp_session_failures
            )

        self.add_attempt_log("TELEMETRY", status)
    ...
    def add_mllog_summary(self, result_text, reason_text, fp_score=0):
        ...
        text = "attempt={};result={};reason={};risk={};score={};fp_score={};total_failed={}".format(
            self.attempt_number,
            result_text,
            reason_text,
            self.current_risk_level,
    ...
    def train_risk_model(self, label):
        ...
        prediction, error = self.risk_engine.train(
            len(self.wrong_pin_times),
            len(self.fingerprint_fail_times),
            len(self.recent_lockout_times),
            rapid,
            repeated,
            label
        )

        self.add_attempt_log(
            "MLTRAIN",
            "label={} pred={:.2f} err={:.2f}".format(label, prediction, error)
        )
    ...
    def update_risk(self, reason=REASON_NONE, lockout_started=False):
        ...
        self.recent_fail_times = [
            t for t in self.recent_fail_times
            if (now - t) <= RISK_FAIL_WINDOW_SECONDS
        ]

        self.recent_lockout_times = [
            t for t in self.recent_lockout_times
            if (now - t) <= RISK_LOCKOUT_WINDOW_SECONDS
        ]

        self.wrong_pin_times = [
            t for t in self.wrong_pin_times
            if (now - t) <= RISK_FAIL_WINDOW_SECONDS
        ]

        self.fingerprint_fail_times = [
            t for t in self.fingerprint_fail_times
            if (now - t) <= RISK_FAIL_WINDOW_SECONDS
        ]
        ...
        self.current_risk_score, self.current_risk_level = self.risk_engine.evaluate(
            len(self.wrong_pin_times),
            len(self.fingerprint_fail_times),
            len(self.recent_lockout_times),
            rapid,
        ...
        self.add_attempt_log(
            "RISK",
            "score={} level={} failed_attempts={} total_failed_attempts={}".format(
                self.current_risk_score,
                self.current_risk_level,
    ...
    def generate_verification_code(self, unix_seconds):
        # Generate a TOTP value for the supplied Unix time
    ...
    def verify_verification_code(self, entered_code):
        # Accept neighbouring time windows to tolerate small clock drift
        now = unix_time_now()
        valid_codes = (
            self.generate_verification_code(now - VERIFICATION_CODE_INTERVAL),
            self.generate_verification_code(now),
            self.generate_verification_code(now + VERIFICATION_CODE_INTERVAL)
        )
    ...
    def verification_code_step(self):
        ...
        while attempts < MAX_VERIFICATION_CODE_ATTEMPTS:
            ...
            if self.verify_verification_code(entered_code):
                ...
                self._log_event(
                    EVENT_VERIFICATION_CODE_VERIFIED,
    ...
    def admin_alert_lockdown(self):
        ...
        self._log_event(
            EVENT_LOCKOUT,
            "event=intrusion_detected;response=lockdown",
            REASON_ADMIN_ALERT,
            "permanent=1"
        )
        ...
        while self.permanent_lockdown:
            ...
            if matched and match_id == ADMIN_FINGERPRINT_ID:
                self.add_attempt_log(
                    "EVENT",
                    "Lockdown lifted - admin biometric score={}".format(fp_score)
                )
                self._log_event(
                    EVENT_ADMIN_ACTION,
                    "event=lockdown_cleared;method=fingerprint_override",
                    REASON_ADMIN_ALERT,
                    "fp_score={};match_id={}".format(fp_score, match_id),
                    fp=fp_score,
                    access_result=1,
                    fa=0
                )
    ...
    def unlock_cycle(self, fp_score):
        # Unlock the system, keep the access window open, then secure it again
        self.add_attempt_log("EVENT", "Access authorised")
        self._log_event(
            EVENT_SYSTEM_UNLOCKED,
            "event=access_granted;state=unlocked",
            extra="fp_score={}".format(fp_score),
            fp=fp_score,
            lock=0,
            access_result=1,
            fa=0
        ...
        self._seal_local_record(
            "event=access_granted;fp_score={};time={}".format(fp_score, unix_time_now())
        )
    ...
    def admin_enrol(self):
        ...
        if ok:
            ...
            self._log_event(
                EVENT_ADMIN_ACTION,
                "event=administrative_action;method=enrolment",
                REASON_ENROL,
                "id={}".format(template_id),
                access_result=1,
                fa=0
            )
            self._seal_local_record(
                "event=admin_enrol;id={};time={}".format(template_id, unix_time_now())
            )
            self.reset_escalation_state()
        else:
    ...
    def admin_clear(self):
        ...
        if ok:
            ...
            self._log_event(
                EVENT_ADMIN_ACTION,
                "event=administrative_action;method=template_clear",
                REASON_CLEAR,
                "id={}".format(template_id),
                access_result=1,
                fa=0
            ...
            self._seal_local_record(
                "event=admin_clear;id={};time={}".format(template_id, unix_time_now())
            )
            self.reset_escalation_state()
            sleep(LCD_INFO_DELAY)
        else:
    ...
    def admin_action(self, fp_score, match_id):
        ...
        self._log_event(
            EVENT_FINGERPRINT_VERIFIED,
            "event=access_attempt;method=fingerprint;outcome=verified",
            extra="fp_score={};match_id={}".format(fp_score, match_id),
            fp=fp_score,
            access_result=1,
            fa=0
        )
    ...
    def _handle_honeypot(self):
        ...
        self._log_event(
            EVENT_FAILED_ATTEMPT,
            "event=deception_triggered;response=honeypot_lockout",
            REASON_WRONG_PIN,
            "honeypot=1"
        )
    ...
    def handle_failure(self, reason):
        ...
        self._log_event(
            EVENT_FAILED_ATTEMPT,
            "event=access_attempt;outcome=denied",
            reason,
            "count={}".format(self.failed_attempts)
        )
        ...
        if self.final_warning_mode:
            ...
            self.add_attempt_log(
                "SECURITY",
                "Final attempts left={}".format(self.high_risk_attempts_left)
            )
    ...
    def run(self):
        ...
        while True:
            ...
            if matched and fp_score < required_fp_score:
                self.add_attempt_log(
                    "SECURITY",
                    "Match rejected - score {} below threshold {}".format(fp_score, required_fp_score)
                )
                matched = False
            ...
            if not matched:
                ...
                self.add_attempt_log(
                    "SECURITY",
                    "FP session failures: {}/{}".format(self.fp_session_failures, FP_MAX_ATTEMPTS)
                )
            ...
            # User path
                ...
                self.add_attempt_log(
                    "EVENT",
                    "Fingerprint verified id={} score={}".format(match_id, fp_score)
                )
                self._log_event(
                    EVENT_FINGERPRINT_VERIFIED,
                    "event=access_attempt;method=fingerprint;outcome=verified",
...
def run_app(telemetry_obj):
    # Bind telemetry and start the main runtime
    global telemetry
